...
class ProtocolOutsourcesController extends AppController
{
    ...
    public function admin_approve_other($id = null)
    {
        ...
        if ($outsource) {
            $this->Outsource->id = $data['Outsource']['id'];
            $this->Outsource->saveField('model_sae', $data['OutsourceRequest']['sae']);
            $this->Outsource->saveField('model_ciom', $data['OutsourceRequest']['ciom']);
            $this->Outsource->saveField('model_dev', $data['OutsourceRequest']['dev']);

            $this->OutsourceRequest->id = $id;
            $this->OutsourceRequest->saveField('status', '1');
        }

        $this->Session->setFlash(__('The application has been successfully assigned to the outsourced user'), 'alerts/flash_success');
        $this->redirect($this->referer());
        // debug($data['Outsource']);
    ...
    public function admin_view($user_id = null, $application_id = null)
    {
        ...
        $user = $this->User->find(
            'first',
            array(
                'contain' => array('ProtocolOutsource' => array('Outsource' => array('PendingOutsourceRequest' => array('Attachment')), 'Application')),
