$(function() {
    ...
    if ($("#managercommentform tr").length == 1 ) { $("#managerTableHeader").hide(); }
    setAttachmentUpload();
    ...
    $("#addCommentAt").click(function() {
        $("#managerTableHeader").show();
        var intId = $("#managercommentform tr").length - 1;
        if ($('#managercommentform :input[type="file"]').length < 4) {
            $("#managercommentform").append(constructATr(intId));
            setAttachmentUpload();
        } else {
    ...
    function constructATr(intId) {
        ...
        <td><div class="control-group">\
            <input type="hidden" id="Attachment{i}Model" name="data[Attachment][{i}][model]" value="Review">\
