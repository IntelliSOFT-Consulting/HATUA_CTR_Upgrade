$(function() {
    ...
    if ($("#buildoutsourceformother tr").length == 1 ) { $("#attachmentsTableHeader").hide(); }
    
    // incremental development
    $("#addSupportingOther").click(function() {
        $("#attachmentsTableHeader").show();
        var intId = $("#buildoutsourceformother tr").length - 1;
        if ($('#buildoutsourceformother :input[type="file"]').length < 4) {
            $("#buildoutsourceformother").append(constructATr(intId));
            setAttachmentUpload();
        } else {
    ...
    function constructATr(intId) {
        ...
        <td><div class="control-group">\
            <input type="hidden" id="Attachment{i}Model" name="data[Attachment][{i}][model]" value="OutsourceRequest">\
