$(function() {
    ...
    if ($("#dsursform tr").length == 1 ) { $("#attachmentsTableHeader").hide(); }
    
    // incremental development
    $("#addDsur").click(function() {
        $("#attachmentsTableHeader").show();
        var intId = $("#dsursform tr").length - 1;
        if ($('#dsursform :input[type="file"]').length < 4) {
            $("#dsursform").append(constructATr(intId));
            setAttachmentUpload();s
        } else {
    ...
    function constructATr(intId) {
        ...
        <td><div class="control-group">\
            <input type="hidden" id="Attachment{i}Model" name="data[Attachment][{i}][model]" value="Safety">\
