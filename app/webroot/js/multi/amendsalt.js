$(function() {
    ...
    if ($("#amendsform tr").length == 1 ) { $("#amendHeader").hide(); }
    setAttachmentUpload();


    function setAttachmentUpload() {
        $(':input:file').each(function() {
            var foreignKey = document.getElementById("foreignKey").getAttribute("data-value");
    
            $(this).fileupload({
                url: '/applicant/amendments/edit_amd/'+foreignKey+'.json', 
                dataType: 'json',
                fileInput: $(this),
                add: function (e, data) {
                ...
                submit: function (e, data) {
                    ...
                    // Replace ApplicationId with foreignKey
                    // fieldData.push({ name: "data[Attachment][foreign_key]", value: foreignKey });
                    ...
                    // Prevent saving records without a valid foreign key
                    if (!foreignKey) {
                        $(this).focus();
                    ...
                    if (data.result.message === 'Success') {
                        data.context.prepend(`
                            <a href="/applicant/attachments/download/${data.result.content.Attachment.id}" class="btn btn-info">
                                ${data.result.content.Attachment.basename}
                            </a>`);
    ...
    $("#addAmend").click(function() {
        $("#amendHeader").show();
        var intId = $("#amendsform tr").length - 1;
        if ($('#amendsform :input[type="file"]').length < 4) {
            $("#amendsform").append(constructATr(intId));
            setAttachmentUpload();
        } else {
    ...
    function constructATr(intId) {
        ...
        <td><div class="control-group">\
            <input type="hidden" id="Attachment{i}Model" name="data[Attachment][{i}][model]" value="Amend">\
            <input type="hidden" id="Attachment{i}ForeignKey" name="data[Attachment][{i}][foreign_key]" value="{foreignKey}">\
             <input type="hidden" id="Attachment{i}Pocket" name="data[Attachment][{i}][pocket_name]" value="amendment_cover_letter">\
       ...
       $('.tiptip').tooltip();
        return trWrapper.replace(/{i}/g, intId)
        .replace(/{i2}/g, intId2)
