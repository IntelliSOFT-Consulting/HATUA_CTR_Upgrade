$(function() {
    ...
    if ($("#lineform tr").length == 1 ) { $("#attachmentsTableHeader").hide(); }
    
    // incremental development
    $("#addLine").click(function() {
        $("#attachmentsTableHeader").show();
        var intId = $("#lineform tr").length - 1;
        if ($('#lineform :input[type="file"]').length < 4) {
            $("#lineform").append(constructATr(intId));
            setAttachmentUpload();
        } else {
