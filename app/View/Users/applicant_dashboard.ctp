...
<section>
  ...
  $stages = $this->requestAction( 
    'applications/stages_applicant/' . key($apps)   //get first element of array
  );
  // debug(key($apps));
  //    debug($stages);
  //    exit;
            ...
            <div>
              ...
              foreach ($meetingDates as $meetingDate) {
                if ($meetingDate['MeetingDate']['approved'] < 1) {
                  echo $this->Html->link(
                    '<li>' . $meetingDate['MeetingDate']['proposed_date1'] . ' <small class="muted">(unsubmitted)</small></li>',
                    array('controller' => 'meeting_dates', 'action' => 'edit', $meetingDate['MeetingDate']['id']),
                    array('escape' => false)
                  );
                } else {
                  echo $this->Html->link(
                    '<li>' . $meetingDate['MeetingDate']['proposed_date1'] . '</li>',
                    array('controller' => 'meeting_dates', 'action' => 'view', $meetingDate['MeetingDate']['id']),
        ...
        <div class="thumbnail">
          ...
          <div class="caption">
            ...
            <ol><?php
                ...
                ?>
                  ...
                  <small>
                    ...
                    if (!empty($application['Application']['trial_status_id'])) {
                      if ($application["Application"]["admin_stopped"] == 1) { ?>
                        <span> <?php echo $trial_statuses[$application['Application']['trial_status_id']]; ?></span>
                        <p> <?php echo $application['Application']['admin_stopped_reason']; ?></p>
                      <?php } else { ?>
                        <span class="xeditable tiptip" data-type="select" data-name="data[Application][trial_status_id]" data-url="/applicant/applications/view/<?php echo $application["Application"]["id"]; ?>" data-pk="<?php echo $application['Application']['id']; ?>" data-original-title="Update trial status">
              ...
              <?php
                  ...
                  if ($application['Application']['submitted']) {
                    ...
                    echo "&nbsp;";
                    echo $this->Html->link(
                      '<i class="icon-upload-alt"></i> Upload E2B CIOMS',
            ...
            <?php echo $this->Html->link(
              '<i class="icon-link"></i> View All Applications',
          ...
          <div class="caption">
            ...
            foreach ($saes as $sae) {
              if ($sae['Sae']['approved'] < 1) {
                echo $this->Html->link(
                  '<li>' . $sae['Sae']['reference_no'] . '</li>',
                  array('controller' => 'saes', 'action' => 'edit', $sae['Sae']['id']),
                  array('escape' => false)
                );
              } else {
                echo $this->Html->link(
                  '<li>' . $sae['Sae']['reference_no'] . '</li>',
                  array('controller' => 'saes', 'action' => 'view', $sae['Sae']['id']),
                  array('escape' => false, 'class' => 'text-success')
                );
              }
            }
