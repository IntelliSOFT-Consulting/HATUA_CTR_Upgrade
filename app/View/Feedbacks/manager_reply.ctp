...
<div class="row-fluid">
	<div class="span12">
		<h3>Contact us <small>:PPB will get back to you using the provided email address.</small> <?php  echo $this->element('google-recommend');?></h3>
		<hr>
	<?php
	...
	<div class="row-fluid">
		...
		<?php
			...
			echo $this->Form->input('foreign_key', array('type' => 'hidden', 'value' => $previous_messages[0]['Feedback']['id']));
			   		...
			   		<?php
			   			...
			   			foreach ($previous_messages as $previous_message) {
			   				...
			   				echo "<dd class='morecontent'>".$previous_message['Feedback']['feedback']."</dd>";
			   				  foreach ($previous_message['Reply'] as $reply) {
			   					echo "<dt class='text-warning'>".$reply['subject']." <small class='muted'> created on ".date('d-m-Y H:i:s', strtotime($reply['created']))."</small></dt>";
			   					echo "<dd class='morecontent text-warning'>".$reply['feedback']."</dd>";
			   				  }
			   				$count++;
			   			}
			   		?>
