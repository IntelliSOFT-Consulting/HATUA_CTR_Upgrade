 ...
 */
	protected function _pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		if ($this->{$check}($model)) {
			...
			//CHANGE
			return $this->Html->tag($tag, $this->Html->link($title, '#', array('escape'=>false)), array_merge($options, compact('escape', 'class')));
		}
	}
 ...
 */
	public function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			// CHANGE
			$out .= $this->Html->tag($tag,  $this->Html->link($params['page'], '#'), array('class' => $currentClass));
