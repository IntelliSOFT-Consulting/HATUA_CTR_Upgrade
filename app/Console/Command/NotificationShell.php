...
/*   public function testEmail(){
   ...
   public function registrationEmail(){
       ...
       $variables = array(
          ...
          'activation_link' => Router::url(array('controller' => 'users', 'action' => 'activate_account', $this->args[0]['User']['activation_key']),
                                                              true)
       );
       ...
       $email->emailFormat('html');
       $email->to($this->args[0]['User']['email']);
       $email->cc(array('pv@pharmacyboardkenya.org', 'info@pharmacyboardkenya.org', $this->args[0]['User']['sponsor_email']));
   ...
   public function ppbNewApplication() {
        ...
        $messages = $this->Message->find('list', array(
                                      'conditions' => array('Message.name' => array('manager_new_application_subject', 'manager_new_application',
    ...
    public function managerApproveApplication() {
        $messages = $this->Message->find('list', array(
             'conditions' => array('Message.name' => array('approve_message_subject', 'applicant_approve_message',
                                        'reviewers_approve_message', 'managers_approve_message')),
             'fields' => array('Message.name', 'Message.content')));

        $my_reviewers = $this->Review->find('list', array(
            'conditions' => array('Review.application_id' => $this->args[0]['application_id'], 'Review.type' => 'request',
        ...
        $variables = array(
                'protocol_no' => $this->Application->field('protocol_no', array('id' => $this->args[0]['application_id'])),
                'approved' =>
                        ($this->Application->field('approved', array('id' => $this->args[0]['application_id'])) == 2) ? 'Approved' : 'Rejected',
              );
        $variables['protocol_link' ] = Router::url(array('controller' => 'applications', 'action' => 'view', $this->args[0]['application_id'],
                                               'applicant' => true), true);
        $save_data[] = array('Notification' => array(
   ...
   public function  newAmndtNotifyApplicant() {
        ...
        $save_data = array();
         $variables = array(
            'amendment_link' => Router::url(array('controller' => 'amendments', 'action' => 'edit', $this->args[0]['id'],
                                           'applicant' => true), true),
            'protocol_link' => Router::url(array('controller' => 'applications', 'action' => 'view', $this->args[0]['application_id'],
                                           'applicant' => true), true),
